...
def _build_single_prompt(user_text: str, meta, offense: str) -> str:
    ...
    return (
        ...
        "- 'summary'는 단순히 사용자의 말을 앵무새처럼 요약하는 필드가 아닙니다! 해당 항목에서 어떤 정보는 확인되었고, 반대로 '어떤 정보가 아직 비어있어서(missing) 이 추가 질문을 던지는 것인지' 그 이유(명분)를 상담원처럼 부드럽게 간략한 1문장으로 짚어주세요.\n"
        ...
        '      "summary": "법률 용어 제외, 경어체로 부족한 정보(질문 이유)를 짚어주는 1문장 요약"\n'
        '    }, ...\n'
        '  },\n'
        '  "details": {\n'
        '    "<detail_id>": {\n'
        ...
        '      "slots": {"<slot>": "present|missing|unclear|unknown", ...},\n'
        '      "evidence": "<짧은 인용 또는 빈 문자열>",\n'
        '      "summary": "법률 용어 제외, 부족한 정보(질문 이유)를 짚어주는 경어체로 1문장 요약"\n'
        '    }, ...\n'
        '  }\n'
        "}\n\n"
        "[elements]\n" + elines + "\n\n" +
...
def compose_complaint(meta, collected: dict, evidence: List[str]):
    ...
    user = (
        ...
        "- 고소장 본문 서술시 '~하였습니다.', '~한 사실이 있습니다.', '~입니다.' 등 당당하고 명확한 서술형 종결어미를 사용하십시오.\n"
    ...
    if getattr(meta, "offense", "") == "fraud":
        user += (
            ...
            "- 온라인 닉네임이나 아이디(예: '또리엄마')는 가해자를 특정하는 중요한 단서이므로 고소장 본문에서 설명시 절대 누락하지 마십시오.\n"
            "- 단, 위 닉네임을 은행 계좌의 '실명 예금주'로 오인하여 기재해서는 안 됩니다.\n"
